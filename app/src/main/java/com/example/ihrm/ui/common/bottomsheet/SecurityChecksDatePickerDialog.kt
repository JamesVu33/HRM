...
fun HrmDatePickerDialog(
    ...
    // initialMillis đổi hoặc mở lại dialog: tạo state mới + tháng hiển thị khớp ngày đang chọn (tránh lệch index/tháng).
    key(initialMillis) {
        val anchorMillis = initialMillis ?: System.currentTimeMillis()
        val datePickerState = rememberDatePickerState(
            initialSelectedDateMillis = anchorMillis,
            initialDisplayedMonthMillis = anchorMillis,
            yearRange = yearRange,
            selectableDates = selectableDates,
        )
