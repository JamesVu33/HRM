...
interface EmployeeApiService {
    ...
    @GET("employees")
    suspend fun getAllEmployees(): Response<ApiResponseDto<List<UserResponseDto>>>

    /** GET /employees/meta - roles, titles, levels for mapping. Use when user level is S1/S2. */
    @GET("employees/meta")
    suspend fun getEmployeesMeta(): Response<ApiResponseDto<UserMetaResponseDto>>

    /** GET /me/employee-info - current user's level, title, roles. */
    @GET("me/employee-info")
    suspend fun getMeEmployeeInfo(): Response<ApiResponseDto<MeEmployeeResponse>>

    /** GET /levels - all levels for mapping levelId -> level (code/name) on dashboard. */
    @GET("levels")
    suspend fun getAllLevels(): Response<ApiResponseDto<List<LevelResponseDto>>>

    /** GET /levels/{id} - level by id. */
    @GET("levels/{id}")
    suspend fun getLevelById(@Path("id") id: Int): Response<ApiResponseDto<LevelResponseDto>>

    /** GET /employees/{id} - response: { statusCode, message, data: { ..., level?: { id, code, name, ... } } }. */
    @GET("employees/{id}")
    suspend fun getEmployeeById(@Path("id") id: String): Response<ApiResponseDto<EmployeeDto>>

    @POST("employees")
    suspend fun createEmployee(@Body employee: EmployeeDto): Response<ApiResponseDto<EmployeeDto>>

    @PUT("employees/{id}")
    suspend fun updateEmployee(
        @Path("id") id: String,
        @Body employee: EmployeeDto
    ): Response<ApiResponseDto<EmployeeDto>>

    @DELETE("employees/{id}")
    suspend fun deleteEmployee(@Path("id") id: String): Response<ApiResponseDto<Unit>>
}
